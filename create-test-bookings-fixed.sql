-- Create test bookings fixed - resolve date type casting error
...
-- 5. Create multiple test bookings with different hotels (fixed date casting)
DO $$
DECLARE
  hotel_ids UUID[];
  ...
  IF array_length(hotel_ids, 1) > 0 THEN
    ...
    FOR i IN 1..LEAST(3, array_length(hotel_ids, 1)) LOOP
      INSERT INTO public.bookings (
        ...
        hotel_ids[i],
        '70a7bc52-5385-4fea-b9f6-653d0e1ede67',
        CURRENT_DATE + INTERVAL '1 day' * i,  -- Fixed: removed ::TEXT casting
        CURRENT_DATE + INTERVAL '3 days' * i, -- Fixed: removed ::TEXT casting
        (1 + i)::INTEGER,
        CASE i
