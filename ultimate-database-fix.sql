-- Ultimate database fix - resolve all submission and admin issues
...
-- 3. Ensure hotel_submissions table exists and has correct schema
...
-- 6. Test hotel submission insert (the exact operation that's failing)
DO $$
DECLARE
  submission_success BOOLEAN;
BEGIN
  -- Try to insert a test hotel submission
  ...
  RAISE NOTICE 'Test hotel submission: %', CASE 
...
SELECT 
  ...
  'Hotel submissions should work now' as submission_status,
  'Admin dashboard should work now' as admin_status;
