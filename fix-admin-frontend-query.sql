-- Fix admin frontend query - resolve 400 error
...
-- 8. Update existing profiles with missing data
DO $$
BEGIN
  -- Update profiles with user data from auth.users
  UPDATE public.profiles 
  SET 
    full_name = COALESCE(full_name, au.raw_user_meta_data->>'full_name', 'User ' || LEFT(au.email::text, 10)),
    email = COALESCE(email, au.email),
    phone = COALESCE(phone, '+977-0000000000')
  FROM auth.users au
